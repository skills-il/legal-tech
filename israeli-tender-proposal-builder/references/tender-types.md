...
- **Typical use:** Specialized goods or services where the pool of capable bidders is small, or where the issuing body has already vetted a list of approved suppliers
...
| Category | Hebrew | Typical conditions |
|----------|--------|-------------------|
| Below threshold amount | התקשרות בסכום נמוך | Contract value below the prescribed threshold |
| Urgent cases | מצב חירום | Immediate need where a public tender would defeat the purpose |
| Continuation of existing engagement | המשך התקשרות | Extension of a prior contract where re-tendering would be impractical |
| Sole source | ספק יחיד | Only one supplier can provide the goods or service |
...
## How to identify the tender type

Look for these clues in the tender document:

1. **Publication channel.** A tender printed in the newspapers or on the government procurement portal is public. A tender delivered by invitation letter is closed.
2. **Section title.** Tender documents usually state the type explicitly in the cover page or general chapter.
3. **Framework language.** Watch for phrases like "הסכם מסגרת", "רשימת ספקים מאושרת", "פניה לקבלת הצעות בתוך מאגר".
4. **Exemption language.** Watch for phrases like "פטור ממכרז", "התקשרות בדרך של פטור", "בירור מחירים".

If the type is unclear, ask in a clarification question. Do not guess.
